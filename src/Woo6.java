...
public class Woo6 {
    ...
    //1회차 도전. 01시 35분 시작. 03시 10분이 되어도 풀지 못했다.
    //2회차 도전. 14시 42분 시작. 15시 30분 완료.
    //배열에 2개로 나눈 문자열 저장.
    //중복체크
    //만약 중복이 체크되면 체크된 배열 인덱스 알아낼 것
    //알아낸 배열 인덱스로 이메일 검출.
    ...
    public String[] solution(String[][] forms) {
        ...
        for (int i = 0; i < forms.length; i++) {
            nickName.add(forms[i][1]);
            for (int j = 0; j < forms[i][1].length() - 1; j++) {
                subStr.add(forms[i][1].substring(j, j + 2));
            }
        }
        ...
        for (int i = 0; i < subStr.size(); i++) {
            ...
            for (int j = 0; j < subStr.size(); j++) {
                if (subStr.get(i).equals(subStr.get(j))) {
                    count = count + 1;
                }
            }
            if (count > 1 && !overlap.contains(subStr.get(i))) {
                overlap.add(subStr.get(i));
            }
        }

        for (int i = 0; i < nickName.size(); i++) {
            for (int j = 0; j < overlap.size(); j++) {
                if (nickName.get(i).contains(overlap.get(j))
                        && !answerList.contains(forms[i][0].replace("@email.com", ""))) {
                    answerList.add(forms[i][0].replace("@email.com", ""));
                }
            }
        }
        //끝에 숫자만 오는 상태에서 정렬을 하면 1, 11, 2, 21 순으로 정렬되지만 끝에 문자열이 오는상태에서 정렬을 하면 11, 1, 21, 2 순으로 정렬된다. 왜?
        Collections.sort(answerList);
        for (int i = 0; i < answerList.size(); i++) {
            answerList.set(i, answerList.get(i).concat("@email.com"));
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println(convertTestCase);
        ...
        str += "]";
        if (str.equals(new BufferedReader(new InputStreamReader(System.in)).readLine())) {
